...
(defmacro rangho-themes-define-theme (name doc-string role-color-alist &rest overrides)
  "Define a theme named NAME with ROLE-COLOR-ALIST.
Like `defun', DOC-STRING can be specified as the second argument, and one can
override the default configuration in OVERRIDES."
  (declare (doc-string 2)
           (indent defun))
  ;; Make sure that all required roles are defined.
  (unless (cl-every (lambda (role)
                      (assoc role role-color-alist))
                    rangho-themes-material-color-roles)
    (error "Not all required roles are defined"))
  (let-alist role-color-alist
    (let* ((term0  (or .term0  .surface))
           (term1  (or .term1  .error))
           (term2  (or .term2  .tertiary))
           (term3  (or .term3  .secondary))
           (term4  (or .term4  .primary))
           (term5  (or .term5  .tertiary-container))
           (term6  (or .term6  .secondary-container))
           (term7  (or .term7  .on-surface))
           (term8  (or .term8  .outline))
           (term9  (or .term9  (rangho-themes-lighten term1 0.1)))
           (term10 (or .term10 (rangho-themes-lighten term2 0.1)))
           (term11 (or .term11 (rangho-themes-lighten term3 0.1)))
           (term12 (or .term12 (rangho-themes-lighten term4 0.1)))
           (term13 (or .term13 (rangho-themes-lighten term5 0.1)))
           (term14 (or .term14 (rangho-themes-lighten term6 0.1)))
           (term15 (or .term15 .on-surface)))
      `(progn
         (deftheme ,name ,doc-string)
         (custom-theme-set-faces
          ...
          '(rainbow-delimiters-depth-7-face ((t (:foreground ,.primary-container))))
          '(rainbow-delimiters-depth-8-face ((t (:foreground ,.secondary-container))))
          '(rainbow-delimiters-depth-9-face ((t (:foreground ,.tertiary-container))))
