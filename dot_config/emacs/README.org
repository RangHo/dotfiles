...
Here, we enable the following switches:
...
Some packages require newer versions of built-in packages.
For these, we're better off manually downloading tarballs from ELPA.
...
#+begin_src emacs-lisp
  (use-package mwheel
    ...
    (mouse-wheel-scroll-amount
     '(3 ((control) . 1) ((shift) . 5))
     "Scroll 3 lines at a time, 1 line with control, and 5 lines with shift.")
...
Editing Unicode-heavy codes kinda sucks if I need to type LaTeX sources every time.
Before defining any input methods, any math-related input method can benefit from the data package ~math-symbol-lists~.
Input method definitions require the data package to be installed at compile time, so we need to have Elpaca wait for its installation.

#+begin_src emacs-lisp
  (use-package math-symbol-lists)
#+end_src

At least for Iris-related sources, I nicked the [[https://gitlab.mpi-sws.org/iris/iris/-/blob/master/docs/editor.md][recommended editor setup guide]] from Iris repository.

#+begin_src emacs-lisp
  (use-package uni-iris
    :ensure nil)
#+end_src
