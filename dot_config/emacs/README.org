...
Before loading any other configuration, 
...
#+begin_notes
Make sure to adjust the ~elpaca-directory~ when updating the installer snippet below!
Without it, Elpaca will attempt to download everything from scratch again...
#+end_notes
...
** Do Not Litter!
Packages tend to do /whatever/ they want in the ~user-emacs-directory~, and it makes things harder for me to manage what to persist or not.
Let's address that.

First, define an extended helper function.
...
Then, make sure that my idea of "no littering" is correctly reflected.
...
#+begin_src emacs-lisp
  ...
  (defun rangho--no-littering-theme-custom-themes ()
  ...
  (defun rangho--no-littering-theme-load-path ()
  ...
  (use-package no-littering
    ...
    (rangho--no-littering-theme-custom-themes)
    (rangho--no-littering-theme-eln-cache)
    (rangho--no-littering-theme-load-path)
...
Using ~elpaca~ will pull the entire Emacs codebase which will cost around 2 GiB per package.
...
#+begin_src emacs-lisp
  ...
  (defun rangho--julia-mode-contact (interactive)
    "Produce the Eglot contact information for Julia language server.
  Ensure that Julia language server is installed when called with INTERACTIVE."
    ...
    (when interactive
      (unless (executable-find "julia")
        (user-error "Julia is not installed"))
      (process-file "julia" nil t nil
                    "--startup-file=no"
