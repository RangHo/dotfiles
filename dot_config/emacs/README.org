...
On Linux systems, I basically only use tiling window managers.
In that case, we can drop the window decorations.
...
*** Opam Switch Integration
opam (OCaml Package Manager) has a feature called "switches", which is an isolated installation of a specific version of the OCaml Compiler.
Essentially, this is a OCaml version of rbenv.
    ...
    (rangho-fonts-fixed-font-alist
     '((nil "hesalche" "semteulche"
       ...
       (kana "Sarasa Mono J"
             ...
             "monospace")
       (han "Sarasa Mono TC" "Sarasa Mono SC"
    ...
    (rangho-fonts-variable-font-alist
     '((nil "Noto Sans" "Noto Serif"
       ...
       (kana "Noto Sans CJK JP" "Noto Serif CJK JP"
             "Meiryo" "Yu Gothic" "Yu Mincho"
             "sans-serif")
       (han  "Noto Sans CJK TC" "Noto Serif CJK TC"
...
#+begin_src emacs-lisp
  (cond
   ...
   ((executable-find "wal")
    ...
    (load-theme 'pywal t))
   (t
    ;; Since no colorscheme generator is found, don't do anything.
    nil))
#+end_src
...
For Coq modes, enable ~opam-switch-mode~.
...
JSON, short for JavaScript Object Notation, is a /de facto/ standard serialization method for multi-platform communications.
...
It has a specific tree-sitter mode.
